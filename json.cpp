...
int Lept_value::lept_parse_array(Lept_context* c) {
	...
	for (;;) {
		...
		if (*c->json == ',') {
			...
			u.a.e = (Lept_value*)malloc(size * sizeof(Lept_value));
	...
	for (;;) {
		if (*c->json == '\"') {
			...
			if ((ret = key.parse(c)) != LEPT_PARSE_OK)
				break;
			lept_parse_whitespace(c);
			size_t len = key.get_string_length();
			Lept_member m;
			m.k = (char*)malloc(sizeof(char) * (len + 1)); //可能存在内存泄漏
			memcpy(m.k, key.get_string(), len);
			m.k[len] = '\0';
			m.klen = len;
			if (*c->json != ':') {
				ret = LEPT_MISS_COLON;
			...
			if (*c->json == ',') {
				...
				c->json++;
				u.o.m = (Lept_member*)malloc(size * sizeof(Lept_member));
